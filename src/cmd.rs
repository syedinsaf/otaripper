...
impl Cmd {
    ...
    fn run_op(
        ...
        match Type::try_from(op.r#type) {
            ...
            Ok(Type::Zero) => Ok(()), // This is a no-op since the partition is already zeroed
            Err(e) => bail!("unrecognized operation type: {:?}. Error: {}", op.r#type, e),
            _ => Ok(()),
        }
    }        
    fn run_op_replace(
    ...
    fn verify_sha256(&self, data: &[u8], exp_hash: &[u8]) -> Result<()> {
        ...
        // Convert the got_hash to a byte slice and encode it as a hex string
        ensure!(
        got_hash.as_slice() == exp_hash,
        "hash mismatch: expected {}, got {}",
        hex::encode(exp_hash),
        hex::encode(got_hash.as_slice())  // This converts got_hash to hex
    );
